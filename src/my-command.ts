...
export default function(): void {
  ...
  const specsFrame = new sketch.Artboard({
    ...
    frame: new sketch.Rectangle(
      ...
      sidebarWidth + 80 + mainFrameWidth + 80,
      specsFrameHeight
    ),
  });
  ...
  const duplicatedArtboard = (originalArtboard as any).duplicate();
  ...
  layers.forEach((node: any, idx: number) => {
    const { layer } = node;
    ...
    const highlightFill = { color: accentColor + '22', fillType: sketch.Style.FillType.Color, enabled: true };
    ...
    const highlight = new sketch.Shape({
      ...
      frame: new sketch.Rectangle(offsetX, offsetY, rect.width, rect.height),
      style: {
        borders: [{ color: accentColor, thickness: 2, enabled: true }],
        fills: [highlightFill],
      },
    });
    (highlight as any).moveToFront();
    (highlight as any).locked = false;
    (highlight as any).hidden = false;
    console.log(`[Specs] Highlight created and moved to front for: ${layer.name}`);
    console.log(`[Specs] Highlight absolute frame: (${highlight.frame.x},${highlight.frame.y},${highlight.frame.width},${highlight.frame.height}) locked: ${(highlight as any).locked} hidden: ${(highlight as any).hidden}`);
    ...
    if (!globalThis._badgePositions) globalThis._badgePositions = [];
    let stackOffsetY = 0;
    let badgeTargetX = badgeX;
    let badgeTargetY = badgeY;
    // Ensure margin from artboard edge
    if (badgeTargetX < badgeMargin) badgeTargetX = badgeMargin;
    if (badgeTargetY < badgeMargin) badgeTargetY = badgeMargin;
    ...
    const badgeShape = new sketch.Shape({
      parent: duplicatedArtboard,
      frame: new sketch.Rectangle(badgeTargetX, badgeTargetY, badgeSize, badgeSize),
      style: {
        fills: [{ color: accentColor, fillType: sketch.Style.FillType.Color, enabled: true }],
        borders: [{ color: '#FFF', thickness: 2, enabled: true }],
      },
    });
    (badgeShape as any).moveToFront();
    (badgeShape as any).locked = false;
    (badgeShape as any).hidden = false;
    const badgeText = new sketch.Text({
      text: `${idx + 1}`,
      parent: duplicatedArtboard,
      frame: new sketch.Rectangle(badgeTargetX, badgeTargetY + 1, badgeSize, badgeSize),
      style: { fontSize: 14, textColor: '#FFF', alignment: 'center' },
    });
    (badgeText as any).moveToFront();
    (badgeText as any).locked = false;
    (badgeText as any).hidden = false;
    console.log(`[Specs] Badge created and moved to front for: ${layer.name}`);
    console.log(`[Specs] Badge shape frame: (${badgeShape.frame.x},${badgeShape.frame.y},${badgeShape.frame.width},${badgeShape.frame.height}) locked: ${(badgeShape as any).locked} hidden: ${(badgeShape as any).hidden}`);
    console.log(`[Specs] Badge text frame: (${badgeText.frame.x},${badgeText.frame.y},${badgeText.frame.width},${badgeText.frame.height}) locked: ${(badgeText as any).locked} hidden: ${(badgeText as any).hidden}`);
  });
  ...
  new sketch.Text({
    text: 'Anatomy',
    parent: specsFrame,
    frame: new sketch.Rectangle(sidebarX + sidebarPaddingX, sidebarPaddingY, sidebarWidth - 2 * sidebarPaddingX, 48),
  ...
  layers.forEach((node: any, idx: number) => {
    ...
    new sketch.Shape({
      parent: specsFrame,
      frame: new sketch.Rectangle(sidebarX + sidebarPaddingX + indent, sidebarYCursor, badgeSize, badgeSize),
      style: {
        fills: [{ color: accentColor, fillType: sketch.Style.FillType.Color, enabled: true }],
        borders: [],
      },
    ...
    new sketch.Text({
      text: `${idx + 1}`,
      parent: specsFrame,
      frame: new sketch.Rectangle(sidebarX + sidebarPaddingX + indent, sidebarYCursor + 2, badgeSize, badgeSize),
    ...
    new sketch.Text({
      ...
      parent: specsFrame,
      frame: new sketch.Rectangle(sidebarX + sidebarPaddingX + badgeSize + 24 + indent, sidebarYCursor, sidebarWidth - sidebarPaddingX - badgeSize - 24 - indent, badgeSize),
    ...
    function prop(label: string, value: string | number) {
      new sketch.Text({
        text: `${label}: ${value}`,
        parent: specsFrame,
        frame: new sketch.Rectangle(sidebarX + sidebarPaddingX + badgeSize + 24 + indent, propY, sidebarWidth - sidebarPaddingX - badgeSize - 24 - indent, propFontSize + 8),
  ...
  sketch.UI.message("Specs Frame created! Sidebar ready for details.");
}